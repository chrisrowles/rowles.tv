...
var notify = {};
notify.toast = sweetalert2__WEBPACK_IMPORTED_MODULE_0___default().mixin({
  toast: true,
  position: 'bottom-end',
...
notify.send = function (type, msg) {
  var position = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 'bottom-end';
  notify.toast.fire({
    position: position,
    icon: type,
    title: msg
...
video.update = function (route, refs) {
  ...
  var handleData = function handleData(data) {
    if (data.errors) {
      ...
      _notify__WEBPACK_IMPORTED_MODULE_0__.default.send('success', 'Video information successfully updated.', 'top');
    }
  };
...
video.preview = function (el) {
  ...
  if (thumbnail) {
    ...
    var isPlaying = preview.currentTime > 0 && !preview.paused && !preview.ended && preview.readyState > preview.HAVE_CURRENT_DATA;
