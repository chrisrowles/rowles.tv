...
video.get = (route, refs) => {
    ...
    const handleData = data => {
        ...
        if (thumbnail && data.metadata.thumbnail_filename) {
            ...
            thumbnail.src = process.env.MIX_CLOUDASSET_IMAGE_URL + '/jpeg/' + data.metadata.thumbnail_filename;
        }
    ...
    fetch(route, {
        ...
        .then(handleResponse)
        .then(handleData)
        .catch(e => notify.send('error', e.message, 'top'));
}
...
video.update = (route, refs) => {
    ...
    const handleData = data => {
        if (data.errors) {
            ...
            notify.send('error', 'There is a problem with the information you have submitted.', 'top');
        } else {
